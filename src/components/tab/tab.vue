    ...
    <div slot="content">
      ...
      <cube-tab-panels v-model="selectedLabel">
        <cube-tab-panel v-for="(item, index) in tabs" :label="item.label" :key="item.label">
          <ul>
            <li class="tab-panel-li" v-for="(hero, index) in item.heroes" :key="index" @click="click">
              {{hero}}
            </li>
          </ul>
        </cube-tab-panel>
...
<script type="text/ecmascript-6">
  import CubePage from '@/components/page/cube-page.vue'
  export default {
    data() {
      return {
        selectedLabel: '天辉',
    ...
    methods:{
        click(e){
            console.log(e.target.innerHTML,"pagex："+e.pageX,"pagey："+e.pageY)
        }
    }
  }
</script>
